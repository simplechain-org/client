...
// In many cases, using CallContract can be preferable to reading raw contracts storage.
...
// CallMsg contains parameters for contracts calls.
type CallMsg struct {
	From      common.Address  // the sender of the 'transaction'
	To        *common.Address // the destination contracts (nil for contracts creation)
	Gas       uint64          // if 0, the call executes with near-infinite gas
	GasPrice  *big.Int        // wei <-> gas exchange ratio
	GasFeeCap *big.Int        // EIP-1559 fee cap per gas.
	GasTipCap *big.Int        // EIP-1559 tip per gas.
	Value     *big.Int        // amount of wei sent along with the call
	Data      []byte          // input data, usually an ABI-encoded contracts method invocation

	AccessList types.AccessList // EIP-2930 access list.
}

// A ContractCaller provides contracts calls, essentially transactions that are executed by
...
// FilterQuery contains options for contracts log filtering.
...
// LogFilterer provides access to contracts log events using a one-off query or continuous
// event subscription.
//
...
// was a contracts creation, the TransactionReceipt method can be used to retrieve the
