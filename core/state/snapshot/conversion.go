...
import (
	...
	"github.com/simplechain-org/client/ethdb"
...
func GenerateTrie(snaptree *Tree, root common.Hash, src ethdb.Database, dst ethdb.KeyValueWriter) error {
	...
	got, err := generateTrieRoot(dst, acctIt, common.Hash{}, stackTrieGenerate, func(dst ethdb.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
		// Migrate the code first, commit the contracts code into the tmp db.
		if codeHash != emptyCode {
			code := rawdb.ReadCode(src, codeHash)
			if len(code) == 0 {
				return common.Hash{}, errors.New("failed to read contracts code")
			}
...
// progressContract updates the generator stats for a specific in-progress contracts.
...
// finishContract updates the generator stats for a specific just-finished contracts.
...
func (stat *generateStats) report() {
	...
	if stat.accounts > 0 {
		...
		if done := binary.BigEndian.Uint64(stat.head[:8]) / stat.accounts; done > 0 {
			...
			// If there are large contracts crawls in progress, estimate their finish time
