...
import (
	...
	"github.com/simplechain-org/client/common/prque"
...
// hasCode reports the contracts code with specific hash is already cached.
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	if s.bloom == nil || s.bloom.Contains(root[:]) {
		...
		// and we hold the assumption that it's NOT legacy contracts code.
...
// AddCodeEntry schedules the direct retrieval of a contracts code that should not
